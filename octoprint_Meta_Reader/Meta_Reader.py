...
"""
def check_pending_threads(self, thread, name):
        if 'pending_threads' in session_saver.saved:
            if name in session_saver.saved['pending_threads']:
                pass
            else:
                Logger.info("Adding a New Thread!")
                session_saver.saved['pending_threads'][name] = thread
        else:
            session_saver.save_variable('pending_threads', {})
            thread.start()
            
            

    def execute_next_thread(self):
        if 'pending_threads' in session_saver.saved:
            if len(session_saver.saved['pending_threads']) > 0:
                for thread in session_saver.saved['pending_threads']:
                    pending = session_saver.saved['pending_threads'][thread]
                    del session_saver.saved['pending_threads'][thread]
                    pending.start()
                    break
            else:
                del session_saver.saved['pending_threads']

    def populate_meta_data(self):

        meta_data = self.meta.detirmine_slicer()
        Logger.info("########################")
        Logger.info(meta_data)

        self.layer_height = meta_data['layer height']
        self.layers = meta_data['layers']
        self.execute_next_thread()
"""
